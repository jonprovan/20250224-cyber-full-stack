...
import com.skillstorm.api.VehicleServiceClient;
...
public class CategoryController {
	...
	// we'll inject our Feign Client here
	private VehicleServiceClient client;
	
	public CategoryController(VehicleServiceClient client) {
		this.client = client;
	}
	...
	// new version using FeignClient
	@GetMapping("/{name}")
	public ResponseEntity<List<Vehicle>> getVehiclesByCategory(@PathVariable String name) {
		...
		return ResponseEntity.ok(client.getVehiclesByCategory(body));
		
	}
	
	// old version using RestTemplate
//	@GetMapping("/{name}")
